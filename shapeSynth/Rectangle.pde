class Rectangle extends Shape {
  ...
  Rectangle(int windowWidth, int windowHeight) {
    //updates the super class constructor 
    super(windowWidth, windowHeight); 

    // set up the synth the shape controls
  ...
  /*
  Rectangle(int windowWidth, int windowHeight, int id, OscP5 osc, NetAddress synthAddress ) {
   //updates the super class constructor 
   super(windowWidth, windowHeight, id, osc, synthAddress); 
  ...
  void updateSynth() {
    ...
    freq = ((float) (windowWidth*windowHeight-sArea))/4000.00;

    println("freq =" + freq);

    // determine the pwm setting.
    if (sHeight > sWidth) {
    ...
    panning = float(centerX)/float(windowWidth);
     ...
     */
    // new OSC message being sent with updated values!
    OscMessage msg = new OscMessage("/updatesynth"); // make osc message
    msg.add(id); // add the shape ID - so that the correct synth can be updated.
    msg.add(freq); // add freq value to the message
    msg.add(pwm); // add pwm value to the message
    msg.add(panning); // add panning value to the message
    osc.send(msg, synthAddress); // sends the message to the synth that needs to be updated.
  }

  // find out the parameter values from the synth.
  void getParametersFromSynth() {
    synth.get("freq", this, "updateFreq");
    synth.get("pwm", this, "updatePwm");
    synth.get("amp", this, "updateAmp");
    synth.get("panning", this, "updatePanning");
  }


  // the following functions map the paramter values from the synth
  // into values that update the shape.
  void updateFreq(int nodeID, String arg, float value) {
     println("frequency: " + value);
  }

  void updatePwm(int nodeID, String arg, float value) {
     println("pwm: " + value);
  }

  void updateAmp(int nodeID, String arg, float value) {
     println("amp: " + value);
  }

  void updatePanning(int nodeID, String arg, float value) {
     println("panning: " + value);
  }
}
