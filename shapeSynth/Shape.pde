class Shape {
  ...
  int windowWidth;
  int windowHeight;
  int id; // this is a numerical identifier for the shape - corresponding to the nodeID of the synth.

  // _REMOVE - remove if all communication can be handled via synth manipulation.
  //OscP5 osc;
  //NetAddress synthAddress;

  Synth synth; // this is the synth object that relates to the shape.

  // default constructor - note the shape needs to know the
  // size of the window - its width and height so it can calibrate itself correctly.
  Shape(int windowWidth, int windowHeight) {

    this.windowWidth = windowWidth; // set the width of the window 
    this.windowHeight = windowHeight; // set the height of the window

    // _REMOVE
    //osc = new OscP5(this, 12000); // make a new Osc object which will recieve on port 12000.
    //superCollider = new NetAddress("127.0.0.1", 57150); // set the (default) address for SuperCollider
  }


  /* 
  // constructor for setting the ID of the shape.
  Shape(int windowWidth, int windowHeight, int id, OscP5 osc, NetAddress synthAddress ) {

    this.windowWidth = windowWidth; // set the width of the window 
    this.windowHeight = windowHeight; // set the height of the window
    this.id = id;    // sets the ID of the shape/

    this.osc = osc; //sets the OSC object in shape.
    this.synthAddress = synthAddress; // sets the netAddress object.
    //osc = new OscP5(this, 12000); // make a new Osc object which will recieve on port 12000.
    //superCollider = new NetAddress("127.0.0.1", 57150); // set the (default) address for SuperCollider
  }
  */

  // Checks if - and if so how the shape should be displayed.
  void display() {
    ...
    if (sWidth == 0 && sHeight == 0) {
      // display boolean and drawShape conditions updated.
      displayShape = false;
      drawShape();
    }
    else {
  ...
  void updateShapeParams() {
    ...
    if (constantCoords[0] > windowWidth) {
      constantCoords[0] = windowWidth;
    }

    if (constantCoords[1] > windowHeight) {
      constantCoords[1] = windowHeight;
    }
    ...
    if (changingCoords[0] > windowWidth) {
      changingCoords[0] = windowWidth;
    }

    if (changingCoords[1] > windowHeight) {
      changingCoords[1] = windowHeight;
    }
